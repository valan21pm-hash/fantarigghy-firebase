...
let pendingFirestoreSyncTask: DatabaseSchema | null = null;
let firestoreSyncTimeout: NodeJS.Timeout | null = null;
let firestoreSyncInProgress = false;
...
async function processPendingFirestoreSync() {
  if (firestoreSyncInProgress || !pendingFirestoreSyncTask) return;

  // Check if data is already in sync
  if (lastSyncedDb && JSON.stringify(lastSyncedDb) === JSON.stringify(pendingFirestoreSyncTask)) {
    console.log("[Firestore Async Sync] No changes detected, skipping.");
    pendingFirestoreSyncTask = null; // Mark as done
    return;
  }

  firestoreSyncInProgress = true;
  const dbToSync = pendingFirestoreSyncTask;
  pendingFirestoreSyncTask = null;

  try {
    console.log(`[Firestore Async Sync] Avvio salvataggio in background...`);
    await saveToFirestore(dbToSync);
    lastSyncedDb = dbToSync; // Update cache
    console.log("[Firestore Async Sync] Backup completato!");
  } catch (err: any) {
    console.error("[Firestore Async Sync] Errore salvataggio:", err);
  } finally {
    firestoreSyncInProgress = false;
    // Sequential fallback: if updates came in while this sync was processing, execute them next
    if (pendingFirestoreSyncTask) {
      firestoreSyncTimeout = setTimeout(async () => {
        firestoreSyncTimeout = null;
        await processPendingFirestoreSync();
      }, 2000); // Wait 2 seconds to avoid exceeding instant write quotas
    }
  }
}
